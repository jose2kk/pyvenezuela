...
from tests.conftest import build_query_id_response_incomplete_text, build_query_id_response_text


def test_query_id__successfull(
    cne_persona_model: cne_schemas.CNEPersonaModel,
    ...
    assert cne_client.query_id(nationality=nationality, number=id) == cne_persona_model
...
def test_query_id__validation_error(
    ...
    assert cne_client.query_id(nationality=nationality, number=id) is None
